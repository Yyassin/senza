...
class DroneDynamics():
    def __init__(self, radius=20., position=np.zeros((2,))):
        self.left = Thruster()
        self.right = Thruster()
        self.thruster_offset = 35.
        self.radius = radius
    ...
    def get_thrust(self):
        ...
        unit_dir = np.array([-np.sin(angle_left), np.cos(angle_left)])

        thrust_left = self.left.get_power() * unit_dir
        thrust_right = self.right.get_power() * unit_dir
    ...
    def update(self, dt):
        ...
        self.angular_velocity += (self.get_torque() * dt - self.angular_velocity * self.air_resistance_coeff)
        self.angle += self.angular_velocity * dt

    def set(self, action=np.zeros((4,))):
        self.left.set_power(0.5 * (action[0] + 1.0))
        self.left.set_angle(action[1])
        self.right.set_power(0.5 * (action[2] + 1.0))
